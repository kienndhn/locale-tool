...
function buildLocalesFromSheet(sheet, options) {
    ...
    const languageHeaders = headerKeys.filter((h) => h !== keyHeader && h !== viHeader);
    const locales = {};
...
function writeLocaleFiles(outDir, locales) {
    ensureDir(outDir);
...
function detectSheet(workbook, sheetName) {
    if (sheetName) {
...
async function main() {
    ...
    const sheetName = args.sheet || undefined;
    const keyCol = args.keyCol || undefined;
    const viCol = args.viCol || undefined;
    if (!input) {
        // eslint-disable-next-line no-console
        console.error("Error: --input=path/to/file.xlsx is required (or provide as first argument)");
        printHelp();
        process.exit(1);
    }
